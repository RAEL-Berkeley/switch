...
class GurobiDirect(DirectSolver):
    ...
    def _get_expr_from_pyomo_repn(self, repn, max_degree=2):
        ...
        for i, v in enumerate(repn.quadratic_vars):
            x, y = v
            new_expr += repn.quadratic_coefs[i] * self._pyomo_var_to_solver_var_map[x] * \
...
class Custom(GurobiDirect):
    ...
    def _set_instance(self, model, kwds={}):
        res = super(Custom, self)._set_instance(model, kwds)
        self._add_scenarios(model)
        return res

    def _apply_solver(self, *args, **kwargs):
        res = super(Custom, self)._apply_solver(*args, **kwargs)
    ...
    def _add_scenarios(self, model):
        ...
        # Change scenario
        gurobi_model.NumScenarios = len(Custom.scenarios)

        old_obj = self._get_objective()

        for i, (name, changes) in enumerate(Custom.scenarios):
            gurobi_model.Params.ScenarioNumber = i
            gurobi_model.ScenNName = name
            for change in changes:
                self.update_param(model, change[0], change[1], change[2])

            # Compute objective function
            new_obj = self._get_objective()

            # See diff
            changes = self._get_changes(new_obj, old_obj)
    ...
    def update_param(self, model, param_name, indexes, value):
        param = getattr(model,  param_name)
        if not hasattr(param, "initial_val"):
            pass
        getattr(model, param_name)[indexes] = value

    def _get_objective(self):
        return generate_standard_repn(self._objective.expr, quadratic=True)

    def _get_changes(self, new_obj, old_obj):
        changes = []
        for i, coef in enumerate(old_obj.linear_coefs):
            new_coef = new_obj.linear_coefs[i]
            if new_coef != coef:
                changes.append((old_obj.linear_vars[i], new_coef))
